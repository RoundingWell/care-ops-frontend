...
const TimestampsView = View.extend({
  ...
  template: hbs`
    <div><h4 class="program-action-sidebar__label">{{ @intl.patients.sidebar.action.activityViews.createdAt }}</h4>{{formatMoment created_at "AT_TIME"}}</div>
    <div><h4 class="program-action-sidebar__label">{{ @intl.patients.sidebar.action.activityViews.updatedAt }}</h4>{{formatMoment updated_at "AT_TIME"}}</div>
  `,
});
