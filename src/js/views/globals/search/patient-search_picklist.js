...
import 'js/components/picklist/picklist.scss';
...
const Picklist = CollectionView.extend({
  ...
  tagName: 'li',
  template: hbs`
    <div class="patient-search__picklist-header">
  ...
  templateContext() {
    const firstPatient = this.collection.first();
    const identifierArray = firstPatient ? firstPatient.get('identifiers') : [];

    return {
      showIdentifierHeader: identifierArray.length > 0,
    };
  },
});
